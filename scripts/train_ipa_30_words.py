...
from src.engines.ipa_engine import create_ipa_engine
from src.data.ipa_dataset import create_ipa_dataloader
...
def setup_mlflow(config: Dict):
    ...
    if mlflow_config.get('enabled', True):
        ...
        mlflow.set_experiment(mlflow_config.get('experiment_name', 'ipa_word_30_training'))
        ...
        mlflow.log_params({
            ...
            'training_approach': 'direct_ipa'
        })

def create_ipa_word_manifest():
    """30개 단어에 대한 IPA 기반 매니페스트를 생성합니다."""
    target_words = [
    ...
    # 전체 매니페스트
    full_manifest_path = os.path.join(output_dir, "manifest_ipa_word_30.json")
    save_json(full_manifest, full_manifest_path)
    
    # 개별 매니페스트
    train_manifest_path = os.path.join(output_dir, "manifest_ipa_word_30_train.json")
    val_manifest_path = os.path.join(output_dir, "manifest_ipa_word_30_val.json")
    
    save_json(train_data, train_manifest_path)
    save_json(val_data, val_manifest_path)
    
    print(f"IPA 기반 30개 단어 매니페스트 생성 완료:")
    ...
    print(f"  - 학습 방식: 음성 → IPA → 텍스트")
    
    # IPA 예시 출력
    print("\nIPA 변환 예시:")
    for i, item in enumerate(manifest_data[:5]):
        print(f"  {item['word']} → {item['ipa']}")
    
    return full_manifest_path, train_manifest_path, val_manifest_path

def train_epoch_with_ipa_tracking(engine, train_loader: DataLoader, config: Dict, 
                                 word_tracker) -> Dict:
    """IPA 추적이 포함된 에포크 훈련을 수행합니다."""
    total_loss = 0.0
    total_steps = len(train_loader)
    
    progress_bar = tqdm(train_loader, desc="IPA 훈련")
    ...
    for step, batch in enumerate(progress_bar):
        ...
        # IPA 성능 추적
        for i, (ref_text, pred_ipa) in enumerate(zip(batch['texts'], batch.get('predicted_ipa', [''] * len(batch['texts'])))):
            if pred_ipa:  # 예측이 있는 경우
                # IPA를 텍스트로 변환하여 비교
...
def evaluate_epoch_with_ipa_tracking(engine, val_loader: DataLoader, config: Dict, 
                                   word_tracker) -> Dict:
    """IPA 추적이 포함된 에포크 평가를 수행합니다."""
    total_loss = 0.0
    total_samples = 0
    
    progress_bar = tqdm(val_loader, desc="IPA 평가")
...
def save_ipa_performance_report(word_tracker, output_dir: str, epoch: int):
    """IPA 기반 단어별 성능 보고서를 저장합니다."""
    ensure_dir(output_dir)
    
    # 성능 데이터 저장
    performance_path = os.path.join(output_dir, f'ipa_performance_epoch_{epoch}.json')
    word_tracker.save_performance_data(performance_path)
    
    # 성능 그래프 저장
    graph_path = os.path.join(output_dir, f'ipa_performance_epoch_{epoch}.png')
    word_tracker.plot_word_performance(graph_path)
    
    # 텍스트 보고서 저장
    report_path = os.path.join(output_dir, f'ipa_performance_epoch_{epoch}.txt')
    report = word_tracker.generate_performance_report()
    
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(report)
    
    print(f"IPA 성능 보고서 저장 완료:")
    print(f"  - 성능 데이터: {performance_path}")
    print(f"  - 성능 그래프: {graph_path}")
    print(f"  - 텍스트 보고서: {report_path}")

def main():
    parser = argparse.ArgumentParser(description="IPA 기반 30개 단어 학습")
    parser.add_argument(
        '--config', 
        type=str, 
    ...
    parser.add_argument(
        '--output_dir', 
        type=str, 
        default='outputs/ipa_word_30_training',
        help='출력 디렉토리'
    )
    parser.add_argument(
        '--create_manifest', 
        action='store_true',
        help='IPA 기반 매니페스트 파일 생성'
    )
    parser.add_argument(
        '--resume', 
        type=str, 
    ...
    try:
        ...
        if args.create_manifest:
            print("IPA 기반 30개 단어 매니페스트 생성 중...")
            create_ipa_word_manifest()
        ...
        with mlflow.start_run():
            ...
            val_loader = create_ipa_dataloader(
            ...
            for epoch in range(max_epochs):
                ...
                # 훈련
                print("IPA 훈련 중...")
                train_metrics = train_epoch_with_ipa_tracking(engine, train_loader, config, word_tracker)
                
                # 평가
                print("IPA 평가 중...")
                val_metrics = evaluate_epoch_with_ipa_tracking(engine, val_loader, config, word_tracker)
                ...
                if (epoch + 1) % save_steps == 0:
                    ...
                    save_ipa_performance_report(word_tracker, output_dir, epoch + 1)
                
                # 최고 성능 모델 저장
                if val_metrics['word_accuracy'] > best_word_accuracy:
                    best_word_accuracy = val_metrics['word_accuracy']
                    best_model_path = os.path.join(output_dir, 'best_ipa_model.pth')
                    engine.save_model(best_model_path)
                    print(f"최고 성능 IPA 모델 저장: {best_model_path}")
                    patience_counter = 0
                else:
                    patience_counter += 1
            ...
            # 최종 모델 저장
            final_model_path = os.path.join(output_dir, 'final_ipa_model.pth')
            engine.save_model(final_model_path)
            print(f"최종 IPA 모델 저장: {final_model_path}")
            
            # 최종 성능 보고서
            save_ipa_performance_report(word_tracker, output_dir, 'final')
            ...
            # MLflow에 최종 모델 저장
            mlflow.pytorch.log_model(engine.model, "final_ipa_model")
            
    except Exception as e:
        print(f"IPA 훈련 중 오류 발생: {e}")
        import traceback
        traceback.print_exc()
        return 1
...
def save_checkpoint(engine, config: Dict, epoch: int, metrics: Dict, 
    ...
    # 모델 저장
    checkpoint_path = os.path.join(checkpoint_dir, f'ipa_checkpoint_epoch_{epoch}.pth')
    engine.save_model(checkpoint_path)
    
    # 메트릭 저장
    metrics_path = os.path.join(checkpoint_dir, f'ipa_metrics_epoch_{epoch}.json')
    save_json(metrics, metrics_path)
    
    # MLflow에 체크포인트 기록
    if mlflow.active_run():
        mlflow.pytorch.log_model(engine.model, f"ipa_checkpoint_epoch_{epoch}")
        mlflow.log_artifact(checkpoint_path)
    
    print(f"IPA 체크포인트 저장 완료: {checkpoint_path}")
