...
class PronunciationDataset(Dataset):
    ...
    def __init__(self, manifest_path: str, config: Dict, 
        ...
        print(f"발음 데이터셋 초기화 완료: {len(self.manifest)}개 샘플")
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # 실제 발음 IPA 타겟 생성
        actual_pronunciation = item.get('actual_pronunciation', item['text'])
        target_ipa = self._create_ipa_target(actual_pronunciation)
        
        # IPA를 인덱스로 변환
        ipa_indices = self._ipa_to_indices(target_ipa)
    ...
    def _create_ipa_target(self, pronunciation: str) -> str:
        """실제 발음을 IPA로 변환합니다."""
        try:
            # 실제 발음을 IPA로 변환
            ipa = self.ipa_converter.text_to_ipa(pronunciation)
            return ipa
        except Exception as e:
            print(f"IPA 변환 오류 ({pronunciation}): {e}")
            # 오류 시 원본 발음 반환
...
def create_pronunciation_dataloader(manifest_path: str, config: Dict, 
                                  batch_size: int = 8, shuffle: bool = True,
                                  num_workers: int = 0) -> DataLoader:
    """발음 데이터로더를 생성합니다."""
    dataset = PronunciationDataset(manifest_path, config, is_training=shuffle)
    ...
    def collate_fn(batch):
        ...
        for item in batch:
            ...
            # 메타데이터
            actual_pronunciations.append(item['actual_pronunciation'])
            expected_texts.append(item['expected_text'])
            audio_files.append(item['audio_file'])
        
        return {
            'audio_features': torch.stack(audio_features),
            'ipa_indices': torch.nn.utils.rnn.pad_sequence(ipa_indices, batch_first=True, padding_value=0),
            'actual_pronunciations': actual_pronunciations,
...
if __name__ == "__main__":
    ...
    # 실제 발음이 포함된 더미 매니페스트 생성
    dummy_manifest = [
        {'audio_file': 'dummy1.wav', 'text': '바지', 'actual_pronunciation': '아지'},
        {'audio_file': 'dummy2.wav', 'text': '가방', 'actual_pronunciation': '가방'},
        {'audio_file': 'dummy3.wav', 'text': '접시', 'actual_pronunciation': '젓시'}
    ]
    ...
    try:
        # 데이터셋 테스트
        dataset = PronunciationDataset(temp_path, config)
        ...
        print(f"실제 발음: {sample['actual_pronunciation']}")
        print(f"기대 텍스트: {sample['expected_text']}")
        print(f"타겟 IPA: {sample['target_ipa']}")
